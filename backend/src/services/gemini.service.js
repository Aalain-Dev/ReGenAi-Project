...
const interviewReportSchema = z.object({
  ...
  technicalQuestions: z
    ...
    .describe("Technical questions that can be asked in the interview along with their intention and how to answer them",),behavioralQuestions: z
    .array(
      z.object({
        question: z
          .string()
...
function isTransientError(err) {
  ...
  return /UNAVAILABLE|overloaded|rate.?limit|temporar/i.test(msg);
}
...
async function callGeminiWithRetry(
  ...
  const modelsToTry = request.model
    ? [request.model, ...models.filter((m) => m !== request.model)]
    : models;
  ...
  const overload = new Error(
    "The AI service is currently overloaded across all fallback models. Please try again in a moment.",
  );
  overload.status = 503;
...
async function generateInterviewReportGemini({
  resume,
  selfDescription,
  jobDescription,
}) {
...
module.exports = generateInterviewReportGemini;
